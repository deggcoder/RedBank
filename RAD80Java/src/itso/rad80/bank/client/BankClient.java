...
public class BankClient {
	...
	private static void executeCustomerTransactions(Bank oBank) throws ITSOBankException {
		try {
			...
			System.out.println("ITSO Bank is opening two new accounts for customer" + customer1 + "...");
			...
			System.out.println("Account " + account11.getAccountNumber() + " and account "
					+ account12.getAccountNumber() + " have been successfully opened for " + customer1 + ".\n");

			// List all accounts for the created customer in the bank
			System.out.println("ITSO Bank - listing all account information of " + customer1 + "...");
			...
			System.out
					.println("\nITSO Bank is going to make credit of $" + amount.setScale(2, BigDecimal.ROUND_HALF_EVEN)
			...
			System.out.println("Account " + account11.getAccountNumber() + " has successfully credited by $"
					+ amount.setScale(2, BigDecimal.ROUND_HALF_EVEN) + ".\n");
			// Prepare debit operation on second account
			amount = new BigDecimal(1234.23D);
			System.out.println("ITSO Bank is going to make debit of $" + amount.setScale(2, BigDecimal.ROUND_HALF_EVEN)
			...
			System.out.println("Account " + account12.getAccountNumber() + " has successfully debited by $"
					+ amount.setScale(2, BigDecimal.ROUND_HALF_EVEN) + ".\n");

			// List current status in all accounts for the created customer in the bank
			System.out.println("ITSO Bank is listing all account information of " + customer1 + "...");
			System.out.println(oBank.getAccountsForCustomer(customer1.getSsn()));

			// Perform automatic close account process on first customer account
			System.out.println("\nITSO Bank is going to close account " + account11.getAccountNumber() + " of customer "
					+ customer1 + "...");
			oBank.closeAccountOfCustomer(customer1, account11);
			System.out.println("Account " + account11.getAccountNumber() + " has been successfully closed for "
					+ customer1 + ".\n");
			// List current status in all accounts for the created customer in the bank
			System.out.println("ITSO Bank is listing all account information of " + customer1 + "...");
			System.out.println(oBank.getAccountsForCustomer(customer1.getSsn()));
			...
			System.out.println("Account " + account12.getAccountNumber() + " has sucessfully credited by $"
					+ amount.setScale(2, BigDecimal.ROUND_HALF_EVEN) + ".\n");
...
//        engine.put("bank", oBank);
		...
		try {
			// Create customer-bank instance with hard coded values
			System.out.println("System is going to add new customer...");
			Customer oCustomer = new Customer("xxx-xx-xxxx", "Mr", "Juan", "Napoli");
			oBank.addCustomer(oCustomer);
			out.println(oCustomer + " has been successfully added.");

			// Add two accounts to the new created customer
			out.println("Opening two new accounts for the customer " + oCustomer + "...");

			// Create and Add first account to the customer
			Account oAccountI = new Account("1", new BigDecimal("10000.00"));
			oBank.openAccountForCustomer(oCustomer, oAccountI);

			// Create and Add second account to the customer
			Account oAccountII = new Account("2", new BigDecimal("11234.23"));
			oBank.openAccountForCustomer(oCustomer, oAccountII);
			out.println("Account " + oAccountI.getAccountNumber() + " and account " + oAccountII.getAccountNumber()
					+ " have been successfully opened for " + oCustomer);

			// List all accounts for the created customer in the bank
			out.println("ITSO Bank is listing all account information of " + oCustomer);
			out.println(oBank.getAccountsForCustomer(oCustomer.getSsn()));

			// Perform an automatic credit operation on both accounts
			// Prepare credit operation on first account
			BigDecimal bgAmount = new BigDecimal("2500.00");
			out.println("\nITSO Bank is going to make credit of $" + bgAmount.setScale(2, BigDecimal.ROUND_HALF_EVEN)
					+ " to account " + oAccountI.getAccountNumber() + "...");
			// Perform credit operation on first account
			oBank.deposit(oAccountI.getAccountNumber(), bgAmount);
			out.println("Account " + oAccountI.getAccountNumber() + " has sucessfully credited by $"
					+ bgAmount.setScale(2, BigDecimal.ROUND_HALF_EVEN) + ".\n");
			// Prepare credit operation on second account
			bgAmount = new BigDecimal("1234.23");
			out.println("ITSO Bank is going to make debit of $" + bgAmount.setScale(2, BigDecimal.ROUND_HALF_EVEN)
			...
			out.println("Account " + oAccountII.getAccountNumber() + " has sucessfully debited by $"
					+ bgAmount.setScale(2, BigDecimal.ROUND_HALF_EVEN) + ".\n");
			...
			// Perform automatic close account process on first customer account
			out.println("\nITSO Bank is going to close account " + oAccountI.getAccountNumber() + " of customer "
					+ oCustomer + "...");
			oBank.closeAccountOfCustomer(oCustomer, oAccountI);
			out.println("Account " + oAccountI.getAccountNumber() + " has been successfully closed for " + oCustomer
					+ ".\n");
			// List current status in all accounts for the created customer in the oBank
			out.println("ITSO Bank is listing all account information of " + oCustomer + "...");
			out.println(oBank.getAccountsForCustomer(oCustomer.getSsn()));
			...
			out.println("Account " + oAccountII.getAccountNumber() + " has sucessfully credited by $"
					+ bgAmount.setScale(2, BigDecimal.ROUND_HALF_EVEN) + ".\n");
			...
			System.err.println("the following application exception occurred ... ");
			e.printStackTrace();
		}
